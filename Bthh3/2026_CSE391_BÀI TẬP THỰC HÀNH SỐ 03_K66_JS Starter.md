...
JavaScript đã được trình duyệt thực thi ngay khi thẻ <script>
 được nạp, chứng tỏ file .js đã kết nối thành công với file .html.
---
...
> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?) Cả hai (Trình duyệt - Client side và Máy chủ - Server side với Node.js)
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Xây dựng khung và nội dung của trang web
> - CSS: Trang trí văn bản , định dạng bố cục 
> - JavaScript: Xử lý logic và tạo tương tác động
...
>   - `number` là:: Kiểu dữ liệu số (bao gồm số nguyên và số thập phân), dùng để tính toán toán học.
>   - `string` là: Kiểu dữ liệu chuỗi (văn bản), luôn được đặt trong dấu ngoặc đơn '', ngoặc kép "" hoặc dấu backtick `.
>   - `boolean` là: Kiểu dữ liệu logic, chỉ có hai giá trị duy nhất là true (đúng) hoặc false (sai).
...
> Câu hỏi:
> - Dòng log hiển thị gì sau khi em sửa thông tin? "Xin chào, mình là [huythuy], năm nay mình [20] tuổi."
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
Trình duyệt sẽ báo lỗi đỏ trong tab Console (vd: Uncaught SyntaxError hoặc ReferenceError). Chương trình sẽ dừng lại và không chạy các dòng lệnh tiếp theo
---
...
> a) Nếu `score = 9`, em dự đoán console sẽ in: Dự đoán: Giỏi | Thực tế: Giỏi
> b) Nếu `score = 6`, em dự đoán console sẽ in: Trung bình | Thực tế: Trung bình
...
> - Khi nào chương trình in `"Giỏi"`? Khi giá trị của biến score lớn hơn hoặc bằng 8.
> - Khi nào chương trình in `"Yếu"`? Khi tất cả các điều kiện phía trên đều sai, tức là khi score nhỏ hơn 5.
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):

cấu trúc if/else giống như một ngã rẽ trên đường đi. Khi bạn đến một ngã ba, bạn sẽ kiểm tra biển chỉ dẫn (điều kiện):

Nếu biển ghi 'Đường đi đến trường' và bạn cần đi học (điều kiện Đúng), bạn sẽ rẽ vào đó.

Nếu không phải (điều kiện Sai), bạn sẽ nhìn tiếp biển tiếp theo hoặc đi thẳng con đường còn lại.
Trong lập trình, nó giúp máy tính biết được nên thực hiện hành động nào tùy thuộc vào dữ liệu đầu vào, giống như việc ta quyết định mang ô nếu trời dự báo có mưa, còn không thì thôi.
...
>   - Tham số (parameters): Các biến nằm trong ngoặc đơn (vd: m1, m2, m3 hoặc avg).
>   - Thân hàm (body): Đoạn mã nằm trong cặp ngoặc nhọn { ... }.
>   - Giá trị trả về (return): Giá trị sau từ khóa return.
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
Ưu điểm: Tái sử dụng code, dễ quản lý, tránh sai sót khi cần sửa logic ở nhiều nơi.
---
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì? : Tìm và chọn phần tử HTML có thuộc tính id là "status"
> - Sự kiện `"click"` xảy ra khi nào? : Xảy ra khi người dùng nhấn chuột trái vào phần tử
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
Nội dung văn bản của thẻ <p id="status"> từ "Chưa có tương tác..." chuyển thành lời chào
---
...
> Câu hỏi:
> - Em có thể đổi sang màu khác (vd. `lightblue`) không? Hãy thử. Có thể đổi thành document.body.style.backgroundColor = "lightblue";
> - Em hãy ghi lại 1 ví dụ khác mà JavaScript có thể làm với `document.body.style`.
document.body.style.fontSize = "20px"; (thay đổi kích thước chữ toàn trang).
---
...
> Câu hỏi:
> - Sự kiện `"input"` khác gì so với `"click"`? "click" chỉ tính 1 lần khi nhấn, "input" xảy ra liên tục mỗi khi người dùng gõ hoặc xóa một ký tự 
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
Dòng greeting sẽ hiển thị: "Xin chào, !" (vì value lúc này là chuỗi rỗng).
---
...
>   : Là một sơ đồ hình cây đại diện cho các thẻ HTML, giúp JavaScript "nắm" được các thẻ đó để thay đổi chúng
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
statusEl.textContent = "..." hoặc document.getElementById("btnRed")
---
...
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì?
 Đều dùng để lắng nghe sự kiện click và thực hiện một hành động (hiện alert).
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
jQuery viết ngắn gọn hơn, dùng ký hiệu $ để chọn phần tử thay vì cụm từ dài dòng của JS thuần
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Đơn giản hóa việc thao tác với DOM và xử lý sự kiện.
>   2. Hỗ trợ tạo hiệu ứng (animation) và gọi API (Ajax) dễ dàng hơn trên nhiều trình duyệt khác nhau.
...
Cách tạo ra các trò chơi nhỏ (Mini game) bằng JS.
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
Một bộ đếm ngược thời gian hoặc nút chuyển đổi chế độ Sáng/Tối (Dark mode).
